...
def send_dine_in_confirmation_email(customer_email, context):
    try:
        # Render the email template with the context
        html_content = render_to_string("email_templates/dine_in_email.html", context)

        # Create the email
        email = EmailMessage(
            subject="Dine-In Reservation Confirmation",
            body=html_content,
