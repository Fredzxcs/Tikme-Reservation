document.addEventListener("DOMContentLoaded", () => {
    ...
    function selectDate(date) {
        ...
        // Display selected date in "Thursday, Feb 08, 2025" format using Asia/Manila timezone
        const formattedDate = date.toLocaleDateString('en-US', {
            weekday: 'long', // "Thursday"
            year: 'numeric', // "2025"
            month: 'short',  // "Feb"
            day: '2-digit',  // "08"
            timeZone: 'Asia/Manila' // Use Manila time zone
        });
    ...
    function convertTo12HourFormat(time) {
        ...
        const formattedTime = `${String(hour % 12 || 12).padStart(2, '0')}:${String(minute).padStart(2, '0')} ${hour >= 12 ? 'PM' : 'AM'}`;
    ...
    async function updateTimeSlots() {
        ...
        for (const time of slots) {
            ...
            const formattedTime = convertTo12HourFormat(time); // Convert time to 12-hour format
    ...
    document.getElementById('continueBtn').addEventListener('click', () => {
        if (selectedDate && selectedPlace && selectedTimeSlot) {
            // Convert the selected date into the formatted string (e.g., "Thursday, Feb 08, 2025")
            ...
            // Format the time into the desired format (e.g., "09:00 AM")
            const formattedTime = convertTo12HourFormat(selectedTimeSlot);
    ...
    document.getElementById('continueBtn').addEventListener('click', () => {
        if (selectedDate && selectedPlace && selectedTimeSlot) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `/dine-in-reservation/?date=${formattedDate}&place=${encodeURIComponent(selectedPlace)}&time=${encodeURIComponent(selectedTimeSlot)}`;
                    window.location.href = url;
                }
            });
        } else {
